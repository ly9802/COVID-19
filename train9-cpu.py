...
dataloader['val']=DataLoader(data_set['val'], batch_size=10, shuffle=False, num_workers=0, collate_fn=None);
...
while (isover==False) and (loss_epoch>0.01):
    ...
    for batch_no, data in enumerate(dataloader['covid19']):
        ...
        #labels=torch.tensor(label_list,dtype=torch.int64).cpu();
        labels=label_list.clone().long();
        ...
        regions_label=torch.tensor(regions_label,dtype=torch.int64);
        loss_batch=0;
    ...
    print("epoch No.",epoch,"------------------------Validation--------------------------");# validation 的意义就是根据预测的acc调整epoch num和loss值
    net.eval();select.eval();
    regions_label=[i for i in range(regions_num)];
    regions_label=torch.tensor(regions_label,dtype=torch.int64); # shape (49)
    predict_epoch=None;
    for batch_no, data in enumerate(dataloader['covid19']):
        img_tensor, label_list=data;
        img_tensor=img_tensor;
...
for batch_no, data in enumerate(dataloader['normal']):       
    ...
    labels=label_list.clone().long().cpu();
    img_featuremap=net(img_tensor); #shape=(batchsize, channels, w,h) 
    batchsize, channel_num, height, width=img_featuremap.shape
...
with open("./region.txt",'w') as f:
    f.write(str(num_selected)+'\n'+str(list(indice2.cpu().numpy()))+'\n')
regions_label.cpu(); accumulation.cpu(); high_frequency_vector.cpu(); regions1.cpu();
...
for epoch in range(epoch_num):
    ...
    for batch_no, data in enumerate(dataloader['val']):
        ...
        predict_label = probability_to_score(predict_probability, threshold).cpu();
        if batch_no==0:
    ...
    print("The epoch is {}, precision:{}, recall:{}, roc_auc:{}, val_acc1:{}".format(epoch, precision,recall,roc_auc,val_acc1),flush=True);
