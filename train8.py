...
backbone_name="resnet18"; regions_num=49;
...
net.cpu();select.cpu();
torch.save(net, './net_model/selector_net_%s.pt'%(backbone_name));
torch.save(select,'./net_model/selector_fc_%s.pt'%(backbone_name));
...
for epoch in range(epoch_num):
    ...
    for batch_no, data in enumerate(dataloader['val']):
        ...
        predict_label = probability_to_score(predict_probability, threshold).cuda();
        if batch_no==0:
